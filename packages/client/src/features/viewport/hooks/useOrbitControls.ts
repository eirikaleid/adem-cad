...
// CAD-standard controls:
//   Middle-drag  -> orbit (inertia)
//   Right-drag   -> pan (inertia)
//   Scroll       -> zoom-to-cursor
//   Shift+scroll -> faster zoom
//   F            -> fit view (smooth lerp)
//   1/3/7/5      -> snap views (smooth lerp)
...
interface OrbitState {
  ...
  targetRadius: number
  orbitVelTheta: number
  orbitVelPhi: number
  panVelX: number
  panVelY: number
  snapActive: boolean
  snapFrom: { theta: number; phi: number; radius: number; target: THREE.Vector3 }
  snapTo: { theta: number; phi: number; radius: number; target: THREE.Vector3 }
  snapT: number
...
function getGroundPoint(mouseNdc: THREE.Vector2, camera: THREE.PerspectiveCamera): THREE.Vector3 | null {
...
export function useOrbitControls(
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (Math.abs(zoomDiff) > 0.001) { s.spherical.radius += zoomDiff * 0.18; dirty = true }
      else { s.spherical.radius = s.targetRadius }
      ...
      if (!s.isOrbiting && (Math.abs(s.orbitVelTheta) > 0.00001 || Math.abs(s.orbitVelPhi) > 0.00001)) {
        ...
        s.orbitVelTheta *= 0.88; s.orbitVelPhi *= 0.88; dirty = true
      }
      ...
      if (!s.isPanning && (Math.abs(s.panVelX) > 0.00001 || Math.abs(s.panVelY) > 0.00001)) {
        ...
        s.panVelX *= 0.88; s.panVelY *= 0.88; dirty = true
      }
      ...
      if (s.snapActive) {
        ...
        if (s.snapT >= 1) {
          s.snapActive = false
          s.spherical.theta = s.snapTo.theta; s.spherical.phi = s.snapTo.phi
          s.spherical.radius = s.snapTo.radius; s.target.copy(s.snapTo.target)
        }
        dirty = true
      }

      if (dirty) applySpherical()
    }
    tick()
    ...
    const startSnap = (toTheta: number, toPhi: number, toRadius?: number) => {
      s.orbitVelTheta = 0; s.orbitVelPhi = 0
      ...
      s.snapT = 0; s.snapActive = true
    }

    const onMouseDown = (e: MouseEvent) => {
      if (e.button === 1) { s.isOrbiting = true; s.orbitVelTheta = 0; s.orbitVelPhi = 0; e.preventDefault() }
      if (e.button === 2) { s.isPanning = true; s.panVelX = 0; s.panVelY = 0; e.preventDefault() }
      s.lastX = e.clientX; s.lastY = e.clientY
      // Double-click: reset view
      if (e.button === 0) {
        const now = Date.now()
        const dist = Math.hypot(e.clientX - s.lastClickX, e.clientY - s.lastClickY)
        if (now - s.lastClickTime < 300 && dist < 8) { s.target.set(0, 0, 0); startSnap(Math.PI / 4, Math.PI / 3.5, 10) }
        s.lastClickTime = now; s.lastClickX = e.clientX; s.lastClickY = e.clientY
      }
    }
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      s.lastX = e.clientX; s.lastY = e.clientY
      if (s.isOrbiting) {
        const dTheta = -dx * 0.006; const dPhi = -dy * 0.006
        s.spherical.theta += dTheta
        s.spherical.phi = Math.max(0.02, Math.min(Math.PI - 0.02, s.spherical.phi + dPhi))
        s.orbitVelTheta = dTheta; s.orbitVelPhi = dPhi; s.snapActive = false
      }
      if (s.isPanning) {
        ...
        _right.crossVectors(_dir, camera.up).normalize()
        _up.crossVectors(_right, _dir).normalize()
        const velX = -dx * panSpeed; const velY = dy * panSpeed
        s.target.addScaledVector(_right, velX); s.target.addScaledVector(_up, velY)
        s.panVelX = velX; s.panVelY = velY; s.snapActive = false
      }
    }

    const onMouseUp = (e: MouseEvent) => {
      if (e.button === 1) s.isOrbiting = false
      if (e.button === 2) s.isPanning = false
    }
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'f': case 'F': s.target.set(0, 0, 0); startSnap(Math.PI / 4, Math.PI / 3.5, 10); break
        case '1': startSnap(0, Math.PI / 2); break
        case '3': startSnap(Math.PI / 2, Math.PI / 2); break
        case '7': startSnap(Math.PI / 4, 0.01); break
        case '5': startSnap(Math.PI, Math.PI / 2); break
      }
    }
